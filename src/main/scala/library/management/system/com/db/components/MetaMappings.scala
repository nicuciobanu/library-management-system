...
import library.management.system.com.model.Models._
...
object MetaMappings {
  ...
  implicit val languageMeta: Meta[Language] =
    Meta[String].timap(name =>
      Language.valueOf(name).getOrElse(throw SerializationError(s"Value $name is not of Language type.")),
    )(_.name)

  implicit val formatMeta: Meta[Format] =
    Meta[String].timap(name =>
      Format.valueOf(name).getOrElse(throw SerializationError(s"Value $name is not of Format type.")),
    )(_.name)
